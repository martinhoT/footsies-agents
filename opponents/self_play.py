...
class SelfPlayManager(OpponentManager):
    ...
    def __init__(
        ...
        Parameters
        ----------
        - `snapshot_method`: producer of agent snapshots to serve as future opponents. May be `None` on initialization, but it *must* be set later before interacting with the environment
    ...
    def update_at_episode(self, game_result: float) -> bool:
        ...
        """
        if self.snapshot_method is None:
    ...
    def set_snapshot_method(self, snapshot_method: SnapshotMethod):
        """Register a method for creating agent snapshots for self-play."""
        self.snapshot_method = snapshot_method
