...
class OpponentDistributionPlot:
    def __init__(
        ...
        self.x_axis: int | str = ""
        self.y_axis: int | str = ""
        self.predicted_series: int | str = ""
        self.estimated_series: int | str = ""
    ...
    def setup(self, title: str, width: int = 1200, other_distribution_name: str = "Action frequencies"):
        ...
        with dpg.plot(label=title, width=width) as plot:
            ...
            self.predicted_series = dpg.add_bar_series((self.x + 1.0 - self.bar_width).tolist(), y.tolist(), label="Predicted", weight=self.bar_width * 2, parent=self.y_axis)
            self.estimated_series = dpg.add_bar_series((self.x + 1.0 + self.bar_width).tolist(), y.tolist(), label=other_distribution_name, weight=self.bar_width * 2, parent=self.y_axis)

            dpg.bind_colormap(plot, dpg.mvPlotColormap_Default)

    def update(self, distribution_predicted: np.ndarray | None, other_distribution: np.ndarray | None):
        if distribution_predicted is not None:
            dpg.set_value(self.predicted_series, [(self.x + 1.0 - self.bar_width).tolist(), distribution_predicted.tolist()])
        if other_distribution is not None:
            dpg.set_value(self.estimated_series, [(self.x + 1.0 + self.bar_width).tolist(), other_distribution.tolist()])
...
class GradientHistogramPlot:
    ...
    def setup(self, title: str, width: int = 1200):
        with dpg.plot(label=title, width=width) as plot:
            ...
            self.histogram = dpg.add_bar_series((self.x + self.step / 2).tolist(), [0.0] * len(self.x), weight=self.step, parent=self.y_axis)
...
class LossPlot:
    def __init__(
        ...
        self.x: list[float] = list(range(max_view_steps))
        ...
        self.x_axis: int | str = ""
        self.y_axis: int | str = ""
        self.current_loss_plot: int | str = ""
        self.threshold_plot: int | str = ""
...
class MimicAnalyserManager:
    def __init__(
        ...
        self.p1_loss_plot = LossPlot(self.p1_model.scars.min_loss, 50) if self.p1_model is not None and self.p1_model.scars is not None else None
        self.p2_loss_plot = LossPlot(self.p2_model.scars.min_loss, 50) if self.p2_model is not None and self.p2_model.scars is not None else None

        # In case we want to act with the predictions, since they are done as simple actions
        self.p2_predicted_action_iterator: Iterator[int] = iter([])

        # DPG items
        self.actual_p1_move: int | str = ""
        self.actual_p2_move: int | str = ""
        self.p1_frameskip: int | str = ""
        self.p2_frameskip: int | str = ""
        self.p1_scar_size: int | str = ""
        self.p2_scar_size: int | str = ""
    ...
    def add_custom_elements(self, analyser: Analyser):
        ...
        with dpg.window(label="Attribute modifier", show=False) as self.attribute_modifier_window:
            def set_scar_min_loss(s, a, u, scar_store: ScarStore):
                scar_store.min_loss = a
            ...
            if self.p1_model is not None:
                ...
                if self.p1_model.scars is not None:
                    dpg.add_slider_float(label="Scars minimum loss", default_value=self.p1_model.scars.min_loss, max_value=10.0, min_value=0.0, width=200, enabled=True, callback=partial(set_scar_min_loss, scar_store=self.p1_model.scars))
            ...
            if self.p2_model is not None:
                ...
                if self.p2_model.scars is not None:
                    dpg.add_slider_float(label="Scars minimum loss", default_value=self.p2_model.scars.min_loss, max_value=10.0, min_value=0.0, width=200, enabled=True, callback=partial(set_scar_min_loss, scar_store=self.p2_model.scars))
        ...
        with dpg.collapsing_header(label="Loss plots"):
            with dpg.group(horizontal=True):
                if self.p1_model is not None:
                    with dpg.group():
                        if self.p1_loss_plot is not None:
                ...
                if self.p2_model is not None:
                    with dpg.group():
                        if self.p2_loss_plot is not None:
        ...
        with dpg.collapsing_header(label="Gradient plots"):
            with dpg.group(horizontal=True):
                if self.p1_model is not None and self.p1_gradient_plot is not None:
                    self.p1_gradient_plot.setup("Player 1 learning gradients", 525)
                if self.p2_model is not None and self.p2_gradient_plot is not None:
    ...
    def on_state_update(self, analyser: Analyser):
        ...
        if analyser.most_recent_transition is not None:
            obs, next_obs, _, terminated, truncated, _, next_info = analyser.most_recent_transition.as_tuple()
            ...
            # Update P1 model
            if self.learn_p1 and p1_simple is not None and self.p1_model is not None:
                self.p1_model.update(obs, p1_simple, terminated or truncated)

            if self.p1_model is not None:
                if self.p1_gradient_plot is not None:
                    self.p1_gradient_plot.update(self.p1_model.network.parameters())
                if self.p1_model.most_recent_loss is not None and self.p1_loss_plot is not None and self.p1_model.scars is not None:
            ...
            # Update P2 model
            if self.learn_p2 and p2_simple is not None and self.p2_model is not None:
                self.p2_model.update(obs, p2_simple, terminated or truncated)
            
            if self.p2_model is not None:
                if self.p2_gradient_plot is not None:
                    self.p2_gradient_plot.update(self.p2_model.network.parameters())
                if self.p2_model.most_recent_loss is not None and self.p2_loss_plot is not None and self.p2_model.scars is not None:
        ...
        if self.p1_mirror_p2 and self.p2_model is not None:
            next_obs_inverted = observation_invert_perspective_flattened(next_obs)
            p1_distribution_predicted = self.p2_model.network.probabilities(next_obs_inverted)[0].squeeze().numpy(force=True)
        elif self.p1_model is not None:
            p1_distribution_predicted = self.p1_model.network.probabilities(next_obs)[0].squeeze().numpy(force=True)
        
        # Get probability distribution of P2 model. This is of the currently-displayed observation
        p2_distribution_predicted = None
        if self.p2_model is not None:
            p2_distribution_predicted = self.p2_model.network.probabilities(next_obs)[0].squeeze().numpy(force=True)

        p1_action_frequencies = self.p1_model.action_frequencies.numpy(force=True) if self.p1_model is not None else None
        p2_action_frequencies = self.p2_model.action_frequencies.numpy(force=True) if self.p2_model is not None else None
    ...
    def p2_prediction_discrete(self, obs: torch.Tensor, info: dict) -> int:
        if self.p2_model is None:
            raise ValueError("cannot perform predictions since P2's model is not defined")
        ...
        try:
            ...
            self.p2_predicted_action_iterator = iter(ActionMap.simple_to_discrete(action)) # type: ignore
            discrete_action = next(self.p2_predicted_action_iterator)
